...
bot.on('guildMemberAdd', member => {
    let channel = member.guild.channels.find('name', 'welcome-leave', '⛔𝐁𝐢𝐞𝐧𝐯𝐞𝐧𝐮𝐞⛔', 'bienvenue', 'bienvenue_adieu👋');
    let memberavatar = member.user.avatarURL
        ...
        .addField(':microphone2: | Salut !', `Welcome to the server, ${member}`)
        .addField(':id: | Utilisateur :', "**[" + `${member.id}` + "]**")
        .addField(':family_mwgb: | Tu es le membre', `${member.guild.memberCount}`)
        .addField("Nom", `<@` + `${member.id}` + `>`, true)
        .addField('Serveur', `${member.guild.name}`, true )
        .setFooter(`**${member.guild.name}**`)
        .setTimestamp()
      member.addRole('561479895629430784', '561598055875346442', '562596663873503237', '561598055875346442')
...
bot.on('guildMemberRemove', member => {
    ...
    let memberavatar = member.user.avatarURL
        ...
        .addField('Bye Bye :(', 'Tu vas nous manquer!')
        .addField('Nous sommes désormais', `${member.guild.memberCount}` + " members")
        .setFooter(`**${member.guild.name}`)
        .setTimestamp()
...
bot.on("message", async message => {
  ...
  if(cmd === `${prefix}kick`){
    ...
    let kickChannel = message.guild.channels.find(`name`, "incidents", "💬𝖉𝖎𝖘𝖈𝖚𝖘𝖘𝖎𝖔𝖓💬", "général");
  ...
  if(cmd === `${prefix}report`){
    ...
    let reportschannel = message.guild.channels.find(`name`, "reports", "💬𝖉𝖎𝖘𝖈𝖚𝖘𝖘𝖎𝖔𝖓💬");
